...
class TransacaoRepository:
    ...
    @staticmethod
    def salvar_motivo(transacao_id, explicacao, score, indicadores):
        conexao = conectar_banco()
        cursor = conexao.cursor()
        cursor.execute("""
            INSERT INTO motivos_analise (transacao_id, explicacao, score, indicadores)
            VALUES (?, ?, ?, ?)
        """, (transacao_id, explicacao, score, ",".join(indicadores) if isinstance(indicadores, list) else indicadores))
        conexao.commit()
        conexao.close()
    ...
    def buscar_motivos():
        ...
        cursor = conexao.cursor()
        cursor.execute("""
            SELECT m.*, t.valor, t.conta, t.cidade 
            FROM motivos_analise m
            JOIN transacoes t ON m.transacao_id = t.id
            ORDER BY m.data_analise DESC
        """)
        linhas = cursor.fetchall()
        conexao.close()
        return [dict(linha) for linha in linhas]
